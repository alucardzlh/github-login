...
app.get('/api/oauth/authorized', async (c) => {
	...
	const env = c.env as Record<string, string>;
	const { GITHUB_CLIENT_ID: client_id, GITHUB_CLIENT_SECRET: client_secret, ENCRYPTION_SECRETS: encryption_password } = env;
	...
	const session = await encodeState(accessToken, encryption_password, Date.now() + TOKEN_VALIDITY_PERIOD);
